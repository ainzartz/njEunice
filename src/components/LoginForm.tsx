      ...
      if (!isMfaStep) {
        ...
        if (res.ok) {
          if (data.mfaRequired) {
            ...
            // Unexpected success without MFA (shouldn't happen with new logic)
        ...
        <div className="w-full max-w-md bg-white p-8 md:p-12 shadow-2xl border border-gray-100">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-light text-gray-900 tracking-wide mb-2">
              {isMfaStep ? 'Verification' : 'Welcome Back'}
            </h1>
            <p className="text-sm text-gray-500 uppercase tracking-widest">
              {isMfaStep ? `Enter the code sent to ${email}` : 'Sign in to your account'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {!isMfaStep ? (
              ...
              <div>
                <label htmlFor="code" className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                ...
                <input
                  ...
                  className="w-full border-b border-gray-300 py-4 focus:border-black focus:outline-none transition-colors text-center text-4xl tracking-[0.5em] font-light text-gray-900"
                  placeholder="000000"
                  required
                  autoFocus
                />
                <div className="mt-6 text-center">
                  <button
                    ...
                    disabled={loading}
                    className="text-xs text-gray-500 hover:text-black underline uppercase tracking-widest transition-colors disabled:opacity-50"
                  >
                    Resend Code
                  </button>
                </div>
            ...
            >
              {loading ? (isMfaStep ? 'Verifying...' : 'Signing In...') : (isMfaStep ? 'Verify & Login' : 'Sign In')}
            </button>
            ...
            {isMfaStep && (
              <button
                ...
                className="w-full text-xs text-gray-400 hover:text-gray-600 uppercase tracking-widest transition-colors mt-2"
              >
                ← Back to Login
              </button>
            )}
